...
- **`/mmry:formation join <id>`** puts this session in a formation, a group of assistants coordinating on one job, so their messages are surfaced here automatically. Also `leave` and `status`.
